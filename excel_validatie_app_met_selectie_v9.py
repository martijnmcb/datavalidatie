...
if uploaded_file:
    ...
    if not xls.sheet_names:
        st.error("⚠️ Het Excel-bestand bevat geen werkbladen. Mogelijk is het opgeslagen als 'Strict Open XML Spreadsheet'."

                 "Open het bestand in Excel en sla het opnieuw op als 'Excel-werkmap (.xlsx)'.")
        st.stop()

    st.subheader("Stap 1: Kies een tabblad om te controleren")
    sheet_name = st.selectbox("Beschikbare tabbladen", xls.sheet_names)
    ...
    if st.button("Start validatie"):
        ...
        if fouten:
            ...
            foutoverzicht = fouten_df['fouttype'].value_counts().reset_index()
            foutoverzicht.columns = ['Fouttype', 'Aantal']
            st.subheader("Overzicht van fouttypes")
            st.dataframe(foutoverzicht)
